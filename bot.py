...
def call_depsearch(query):
    try:
        url = f"{DEP_SEARCH_BASE_URL}/quest={query}&token={DEP_SEARCH_TOKEN}&lang=ru"
...
def call_smsc(phone):
    try:
        url = f"{SMSC_URL}?get_operator=1&login={SMSC_LOGIN}&psw={SMSC_PSW}&phone={phone}"
        r = requests.get(url, timeout=10)
        return _safe_json(r)
    except Exception as e:
        return {"error": str(e)}
...
def run_search(text):
    ...
    if info["tg_combined"]:
        ...
        result["results"]["smsc_operator"] = call_smsc(phone)
        result["results"]["htmlweb_geo"] = call_htmlweb_geo(phone)
        result["results"]["leakcheck"] = call_leakcheck(phone)
        result["results"]["ofdata"] = call_ofdata(phone)
...
def send_result_file(chat_id, query, data, caption=" Результаты поиска"):
    ...
    file_name = f"result_{safe_name}_{timestamp}.txt"
